...
**Correction (recorded during implementation of the final-review fix):** both the "holds a mutex longest" claim in the opening paragraph above and the "longest hold in the system" claim in this paragraph are false, and were found false by reading the code. `withLock` (`run-store.js:593-615`) acquires the lock, runs **one** callback, and releases in the same call; it is used only inside `create`/`update`/`appendEvent`/`writeAssignment`/`writePrivateFile` (`:849`, `:876`, `:900`, `:974`, `:1002`), each its own acquire-and-release. `launch.js` takes the lock five to seven times (`:602` `create`, `:608` `writeAssignment`, `:617` `writePrivateFile`, `:647` `update`, `:676` `writePrivateFile`, `:736` `update`, `:772` `update`) and runs `executeStart` — the git worktree work, the Herdr calls, the agent startup — at `:707`, **outside every one of them**. No lock is held for more than one short local write; there is no "longest hold." The conclusion survives on a different argument: launch is the command that acquires the run lock the most times across the longest wall-clock window, and it is the only path that acquires it on a brand-new run directory (`store.create`'s only caller is `launch.js:602`) — so it has the most opportunities to be killed between the `mkdir` that takes the lock and the `rmdir` that releases it. That is what makes its residue the most likely to need `workflow unlock`.
