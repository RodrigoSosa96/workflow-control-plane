...
**Correction (recorded once implementation task 3 shipped; also see Verification item 2 below):** the synopsis above is stale. The shipped surface takes project as a positional, not a `--project <alias>` flag — `workflow runs [project] [--state <state>] [--all] [--format compact|json]` — following the only precedent this CLI already has for optional project scoping (`doctor`, `reconcile`). `--project` is a hard `Unknown option` today. The implementation's choice is correct; only this document went stale.
...
**Correction (branch review, re-measured):** "the high teens to twenties" above was arithmetic, not measurement, and it undercounted the marginal cost of a run inside the `runs` array — a standalone record's projected size (563–942 characters, measured in isolation) is not what one more entry costs once it sits behind two extra indent levels and array separators inside `JSON.stringify(..., null, 2)`. Re-measured two ways, both driving `formatWorkflowResult("runs", …, "json")` directly rather than estimating from a single record: (1) against this real state root, cycling its 8 actual records (6 three-repo, 2 single-repo) to larger `n` — fits through `n=12` (11,879 characters, 99.0% of budget), collapses to the zero-run-data fallback at `n=13`; cycling only the 6 three-repo records (today's dominant shape) fits through `n=11`, collapses at `n=12`. (2) Against the correspondingly corrected `heavyRunRecord` test fixture (`test/workflow-format.test.js` — it used to carry a single 26-character repository path, understating its own per-run cost by about half; see that test's own correction comment) — three repositories at realistic Herdr worktree depth fit through `n=14` (11,833 characters, 98.6%), collapsing at `n=15`; slightly more headroom than the real data because the fixture's `directory` field is shorter than this installation's actual state-root path. Both measurements agree closely enough to state the honest number as **roughly 12–14 runs**, not the high teens to twenties. The developer is at 8 today: **four to six runs of headroom, not nine to fourteen.**
...
2. `--project` narrows to that project; a project with no runs yields an empty board, not an error. **Correction:** superseded — see the correction under the synopsis above; the shipped surface is a positional `[project]`, not `--project`. The rest of the criterion holds: a project with no runs still yields an empty board, not an error.
