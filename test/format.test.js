...
test("formats triage rows with section and assignee context", () => {
  const output = formatResult("triage", {
    ...
    sections: [{ name: "Doing" }],
    tasks: [{ gid: "t1", name: "Fix OCR", completed: false, sectionNames: ["Doing"], assignee: { name: "Rodrigo", gid: "u1" }, due_on: "2026-07-20", modified_at: "2026-07-17T01:00:00Z", permalink_url: "https://asana.test/t1" }],
  });
  assert.match(output, /Project: OCR \[p1\]/);
  assert.match(output, /Sections: Doing/);
  assert.match(output, /t1 \| Fix OCR \| open \| Doing \| Rodrigo \[u1\] \| due 2026-07-20/);
...
test("formats complete task context while omitting empty sections", () => {
  ...
  assert.match(output, /Assignee: Rodrigo \[u1\]/);
  assert.match(output, /Due: 2026-07-20/);
  assert.match(output, /Parent: Parent \[parent\]/);
...
test("formats attachment metadata with host and links", () => {
  const output = formatResult("attachments", [{ gid: "a1", name: "mock.png", host: "asana", view_url: "https://asana.test/a1" }]);
  assert.equal(output, "mock.png [a1] | asana | https://asana.test/a1");
});
