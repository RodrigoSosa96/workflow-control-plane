...
- There are exactly two documented exceptions to that no-cleanup policy, and neither one is automatic. First, `workflow unlock <run-id>` and `workflow delegation gate-clear <project>` each remove one piece of crash residue (a wedged run lock or reservation gate), and only against a mutex owner proven dead. Second, `workflow archive <run-id>` reclaims one finished run's git worktrees and its Herdr tab — it preserves the run directory, the branch, and every commit any ref points at, it never passes `--force`, and it refuses outright if any worktree holds uncommitted or untracked work. It does delete a worktree's **gitignored** content, which `git status --porcelain` does not report and `git worktree remove` removes silently; the preview names those entries (capped for printing) and binds every one of them into the approval digest rather than refusing. See "Reclaiming a finished run's worktrees" below.
...
- The second exception is `workflow archive <run-id>`, which reclaims one finished run's git worktrees and its Herdr tab. It **preserves the run directory** (`run.json`, `events.jsonl`, `assignment.md`, results — so `workflow result <run-id>` keeps answering forever), and it preserves the **branch and every commit any ref points at**: `git worktree remove` deletes no branch ref, the one case where it could strand a named commit — a detached HEAD no ref contains — is refused outright, and this command never passes `--force`, in any form. It refuses the whole run — removing nothing from any repository — if a worktree holds uncommitted or untracked work, is stopped inside an unfinished git operation, is on a detached HEAD whose commits no ref contains, or is also recorded by another run that is still live. It does delete gitignored content, which it names in the preview and binds into the digest rather than refusing. It is digest-gated like `workflow merge`, and it archives only a run nothing is still working on: a live run state refuses, a held run lock refuses unless its owner is provably dead, and a live Herdr agent refuses. See "Reclaiming a finished run's worktrees" below.
...
When that verification **passes and its evidence was persisted**, `workflow verify` also starts one advisory post-verify critic: a background `code-reviewer` Pi delegation (origin `system-post-verify`, concurrency 1, no remediation turns) that compares the run's registered worktrees against the approved `assignment.md` and reports findings with severity `aside`/`concern`/`blocker`. It is evidence, never a gate: it changes no run state, no exit code, and nothing about `merge` or `archive`, and a `blocker` means "a human should look", not "something was refused". A failed or refused verify — or one whose evidence could not be persisted — starts no critic, and a critic that fails to start lands as `critic: {status: "failed"}` on the verify response without moving `passed` or the exit code. `workflow result <run-id>` shows the latest critic beside the verification claim/proof, marked `stale` whenever a newer verification (passed or failed) supersedes the one it reviewed; `workflow delegation result <run-id> <delegation-id>` keeps the full detail. See `docs/superpowers/specs/2026-08-26-post-verify-critic-design.md`.
...
- **Conflicts are predicted for every repository before anything runs**, with `git merge-tree --write-tree` (git >= 2.38), which performs the real three-way merge machinery without reading or writing any working tree, index, or ref. Any conflict blocks execution for the whole run. A `merge-tree` that cannot answer is a refusal, never "no conflicts" — as is a dirty base checkout, a base checkout whose `git status` cannot be read, a base checkout that is not on `base_branch`, a base checkout stopped inside **any** unfinished git operation (merge, rebase, am, cherry-pick, revert, bisect — the same probe `workflow archive` uses, so an interrupted `rebase -i` with a clean tree is caught too, and the preview names **that operation's own** remedy, e.g. `git -C <path> rebase --abort`, rather than pointing at the dry-run that just printed it), or a base checkout whose operation state cannot be read at all, since an unknown state is a conflict and never clean.
...
- **The branch, and every commit any ref points at.** `git worktree remove` deletes no *branch* ref — but it does delete the worktree's own `HEAD` and per-worktree reflog, which can be the only things referencing a commit. So the guarantee does not come from git: it comes from the refusal below, and it is exactly this wide — **no commit that any ref points at is destroyed.** A worktree on a detached HEAD whose commits no ref contains is refused outright. A commit reachable *only* from the per-worktree reflog is **not** covered: make a commit on a branch and `git reset --hard HEAD~1`, and the worktree is clean, the branch no longer points at that commit, `for-each-ref --contains` finds nothing, and removing the worktree takes the reflog with it. That case is deliberately not gated — every `--amend`, rebase and reset would trip it, and gating on it would refuse nearly every real worktree. Archiving can orphan a directory; it cannot orphan a commit anything still names.
- **The run lock**, which stays `workflow unlock`'s to remove, and every reservation gate.

**It never forces.** There is no `--force`, in any form, and nothing in the command can reach one. A worktree holding uncommitted or untracked changes refuses — for the **whole run**, removing nothing from any of its repositories, because a half-archived group project leaves residue harder to reason about than the original. The refusal names the files, and the remedy it offers for untracked-only work is `git clean -nd`, always with `-n`: a command that refuses to destroy untracked work must not hand you a one-liner that destroys it either. The honest asymmetry, stated plainly: **uncommitted changes refuse; unmerged commits and ignored files only warn.** The line between those first two is recoverability — uncommitted work has no other copy, while unmerged commits are still on a branch in the repository. Ignored files sit on the wrong side of that line and are the exception, covered next.

**Gitignored content is deleted, and the preview names it.** `git worktree remove` without `--force` refuses modified and untracked files — and deletes **ignored** ones silently, exit `0`, no output. `git status --porcelain`, which is what "uncommitted or untracked" means, excludes ignored entries by definition, so that phrase is *not* the complete set of what lives only inside a worktree: a `.env`, a `*.pem`, a local database file, anything an ignore rule covers. This command reads `git status --ignored=matching` as a separate probe, reports each repository's ignored entries as their own kind of loss with the files and the directories counted apart (`node_modules/` is regenerable noise; a `.env` is an only copy), and binds the **full list of entry names** into the approval digest — so renaming or adding one invalidates the approval even if the count is unchanged. It does not refuse on them: five of the eight real worktrees on the machine this was built against carry ignored content and every Node project has `node_modules/`, so refusing would block essentially every real archive. An unreadable ignored probe *does* refuse, because an approval cannot name what it could not read.

**The preview says what would be lost, not just what would be removed.** For every repository it reports how many commits are on the run's branch that `base_branch` does not have, and that count enters the approval digest. A count that could not be measured is reported as `UNKNOWN`, never as `0` — `0` means "checked, and fully merged", and the two must never render the same way. Approving an archive means approving it *with these commits unmerged, and with this ignored content deleted*.
...
- A worktree path that **another still-live run also records** is refused, naming that run — and the remedy points at *it*, not at the run being archived. This is not hypothetical: the worktree path template derives from project + ticket + slug, so relaunching a run reuses the same directory. Measured on the machine this was built against, two pairs of its eight real runs record byte-identical worktree path sets (one pair is a `failed` run beside its relaunch). The dangerous arrangement is the one a relaunch passes through — the original `failed` and archivable, the relaunch still `running` — where archiving the original deletes the live run's working directory. A sharer that is *not* live is a named warning instead, and the set of sharing runs is bound into the digest so an approval goes stale the moment a relaunch starts.
