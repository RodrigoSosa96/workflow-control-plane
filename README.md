...
- The second exception is `workflow archive <run-id>`, which reclaims one finished run's git worktrees and its Herdr tab. It **preserves the run directory** (`run.json`, `events.jsonl`, `assignment.md`, results — so `workflow result <run-id>` keeps answering forever), and it preserves the **branch and every commit**: `git worktree remove` deletes no branch ref, the one case where it could strand commits — a detached HEAD no ref contains — is refused outright, and this command never passes `--force`, in any form. It refuses the whole run — removing nothing from any repository — if a worktree holds uncommitted or untracked work, is stopped inside an unfinished git operation, or is on a detached HEAD whose commits no ref contains. It is digest-gated like `workflow merge`, and it archives only a run nothing is still working on: a live run state refuses, a held run lock refuses unless its owner is provably dead, and a live Herdr agent refuses. See "Reclaiming a finished run's worktrees" below.
...
- **The branch, and every commit on it.** `git worktree remove` deletes no *branch* ref — but it does delete the worktree's own `HEAD` and per-worktree reflog, which on a detached HEAD can be the only two things referencing its commits. So the guarantee does not come from git: it comes from the refusal below. A worktree on a detached HEAD whose commits no ref contains is refused outright, and every other shape has a ref pointing at its work, which survives untouched. Archiving can orphan a directory; it is not allowed to orphan a commit.
