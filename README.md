...
- In other words, every mutating launcher command that changes what's approved or running requires explicit confirmation or --yes. `workflow verify` is the deliberate exception: it mutates only the run's own evidence log, never what's approved or running, and its cost is bounded by its own timeout rather than requiring a gate — the same no-gate posture as the read-only commands above.
