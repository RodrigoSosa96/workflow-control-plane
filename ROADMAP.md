...
2. **1.4** (módulo de invariantes compartidos) — **próximo paso**, ahora que 1.1b está completo. **Encuadre corregido (verificado 2026-07-30):** el valor de 1.4 es prevención de drift y corrección por construcción, **no** cerrar un agujero vivo. La copia débil del predicado de reserva (`coordinator-policy.js` nunca compara el `checkoutDigest` contra el cwd de la delegación) hoy es **inalcanzable**: su único llamador vivo (`executeApproved`) le pasa una reserva que acaba de crear con el `resourceList` autoritativo, y el otro llamador —el gate de `tool_call` de `subagent` en la extensión del coordinador— tiene `getPreparedSubagentContext` como no-op por defecto, así que `validateSubagentRequestPolicy` rechaza de entrada. Sigue valiendo hacerlo: tres copias de un predicado que **ya divergieron** (una es más débil), más ~70 líneas de coreografía de remoción duplicadas que ya exigieron un fix sincronizado durante 1.1. El clasificador de ownership de 1.1 es su primer residente natural, junto a los predicados de reserva y claim token que hoy viven duplicados. La review final de 1.1 encontró un segundo residente concreto y ya verificado: `removeLock` (`run-store.js`) y `clearGate` (`delegation-reservations.js`) duplican ~70 líneas de coreografía de remoción casi idéntica (inspección interna → `allow` → recheck de identidad+bytes → guarda de entrada única → unlink → stat post-unlink → rmdir), que ya exigió un fix sincronizado entre ambos archivos durante este mismo plan (`dc55ba4`, preservar evidencia de ownership cuando una entrada extraña bloquea el rmdir). Hacerlo ahora evita que esa duplicación se convierta en una tercera copia divergente.
