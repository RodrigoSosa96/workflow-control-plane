...
- [ ] **1.1b — Ownership en los locks que toman los hooks** (hueco conocido, encontrado verificando la review final; **no** es una regresión, es alcance que 1.1 no cubrió). Cada proceso que construye su propio run store queda sin el reader de ownership: `hooks/claude-lifecycle.mjs:54`, `hooks/codex-lifecycle.mjs:49`, `hooks/claude-statusline.mjs:123`, `.pi/extensions/workflow-worker-lifecycle.ts:17`, `.pi/extensions/workflow-worker-observability.ts:66` y `scripts/smoke-workflow-fixture.js:171`. `lifecycle-hook-core.mjs:61` llama `store.update`, que toma el lock y escribe el marker — así que un lock tomado por un hook no lleva `pid`/`startedAt` y `workflow unlock` rechaza ese residuo como `unprovable`. Es la misma clase de defecto que el Critical que encontró la review final en `launchCommand`.
  **La tensión de diseño (por eso no se parchó al mergear):** los hooks disparan en cada prompt y cada stop de cada worker, y leer el propio start time significa spawnear `ps` ahí. La review de la tarea 2 movió justamente ese `await` *fuera* de la sección crítica para proteger el presupuesto de retry por contención del lock; meter un spawn de `ps` por prompt en un hook fire-and-forget cuya restricción de diseño es "nunca frenar ni romper al worker" es una decisión que necesita spec, no un patch de último momento. Atenuantes: los hooks sostienen el lock durante un solo merge de campos (milisegundos), contra `launch` que lo sostiene a través de creación de worktree + llamadas a Herdr + arranque del agente; y el estado actual es estrictamente mejor que antes de la rama, cuando **ningún** marker llevaba evidencia.
  **Opciones a evaluar en el spec:** (a) que el hook herede `{pid, startedAt}` por env desde el proceso que lo lanzó — `runEnv` ya inyecta las variables `WORKFLOW_*`, y el start time del *worker* es lo que en realidad interesa; (b) leer el propio start time desde `/proc/self/stat` sin spawnear `ps` (Linux-only, sin costo de proceso); (c) aceptar el hueco y documentar que los locks tomados por hooks se recuperan a mano. La opción (a) parece la más alineada con el diseño y no cuesta un spawn.
